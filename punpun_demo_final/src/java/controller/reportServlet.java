...
public class reportServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String cmd = "Insert into report_log(reason,member_id,project_id) values (?,?,?)";
            insertData = conn.prepareStatement(cmd);
            insertData.setString(1, reason);
            if (reason.equals("อื่น ๆ")) {
                insertData.setString(1, detail);
            }
            insertData.setInt(3, project.getProjectId());
            insertData.setInt(2, mem.getMemberId());
